...
function state(pits: number[], turn: Player = "A", captured: [number, number] = [0, 0]): GameState {
  return {
    board: { pits },
    turn,
    ruleset: "anan_anan",
...
describe("Anan-Anan rules", () => {
  ...
  it("rejects sowing from an opponent pit", () => {
    ...
    expect(() => play(game, { pit: 6 })).toThrow("Pit is not owned by the player to move")
  })
  ...
  it("rejects a move on a finished game", () => {
    ...
    expect(() => play(finished, { pit: 1 })).toThrow("Game is already finished")
  })
  ...
  it("ends the game when a capture passes twenty-four", () => {
    const nxt = play(state([4, 2, 2, 2, 3, 2, 2, 2, 2, 2, 2, 1], "A", [22, 0]), { pit: 0 })
