...
function greedySimulate(state: GameState): Array<[number, number, number]> {
  ...
  for (const pit of movablePits(state)) {
    ...
    gains.push([
      ...
      capturedBy(after, turn === "A" ? "B" : "A") -
        capturedBy(state, turn === "A" ? "B" : "A"),
    ]);
  }
  return gains;
}
...
function greedyResponseSimulate(
  state: GameState,
...
function evaluate(state: GameState, player: "A" | "B"): number {
  ...
  const capturedDiff =
    capturedBy(state, player) - capturedBy(state, player === "A" ? "B" : "A");
  const sideDiff =
    own.reduce((s, v) => s + v, 0) - opp.reduce((s, v) => s + v, 0);
  const mobilityDiff =
    own.filter((v) => v > 0).length - opp.filter((v) => v > 0).length;
  return (
    CAPTURE_WEIGHT * capturedDiff +
    SIDE_WEIGHT * sideDiff +
    MOBILITY_WEIGHT * mobilityDiff
  );
}
