...
const MODE_OPTIONS = ["human", "random", "greedy", "greedy_response", "minimax"] as const
...
export function renderEditButton(onEdit: () => void, enabled: boolean): HTMLElement | null {
...
export function collectBoardEdits(root: ParentNode): EditRead {
  ...
  for (const input of root.querySelectorAll<HTMLInputElement>("input[data-pit]")) {
    pits[Number(input.dataset.pit)] = input.valueAsNumber
  }
  const capturedValue = (p: Player): number => {
    const input = root.querySelector<HTMLInputElement>(`input[data-captured="${p}"]`)
...
export function renderRulesModal(ruleset: Ruleset, onClose: () => void): HTMLElement {
  ...
  title.textContent = ruleset === "anan_anan" ? "Anan-Anan rules" : "Abapa rules"
...
export function renderBoard(
  ...
  wrap.appendChild(renderRow(11, 6, state, "B", onPitClick, "row-top", editing))
  wrap.appendChild(renderRow(0, 5, state, "A", onPitClick, "row-bottom", editing))
  ...
  for (const pit of indices) {
    ...
    if (editing) {
      ...
      if (state.turn === player && state.status === "ongoing" && seeds > 0) {
        ...
        cell.setAttribute("aria-label", `Player ${player}, ${seeds} seeds, move`)
...
export function renderForecast(state: GameState, isHumanTurn: boolean): HTMLElement | null {
...
function renderCaptured(player: Player, count: number, cls: string, editing: boolean): HTMLElement {
...
export function renderStatus(state: GameState, modeA: SideMode, modeB: SideMode): HTMLElement {
...
function modeSelect(current: SideMode, onChange: (m: SideMode) => void): HTMLSelectElement {
